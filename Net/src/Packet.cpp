...
PacketS::PacketS(std::size_t len) : PacketBase(len)
{
	m_PacketSize = len;
}
...
//////////////////////////////////////////////////////////////////////////
PacketC::PacketC(std::size_t len) : PacketBase(len)
{
	m_DataSize = len;
}

PacketC::PacketC(std::size_t data_size, std::size_t ex_data_size) : off_set_(sizeof(uint16_t)), PacketBase(data_size + ex_data_size)
{
...
//Packet::Packet(std::size_t len) : m_MaxSize(len)
//{
...
//}
//
//Packet::Packet(gpb::Message& msg, int nReceiverCount /* = 1*/)
//{
//    assert(msg.ByteSize() <= MAX_MSG_SIZE);
//    assert(nReceiverCount <= MAX_AOI_OBJ_COUNT);
//    // if you want, you can use full_name() to replace it;
//    const std::string& msgName = msg.GetDescriptor()->name();
//    assert(msgName.size() + 1 <= 0xFF);
//    const uint8_t name_len = static_cast<uint8_t>(msgName.size() + 1);  //'\0'
...
//    m_PacketSize = HEADER_SIZE + total_data_size + sizeof(int);
//    m_DataSize = m_PacketSize - sizeof(uint16_t);
//    m_MaxSize = sizeof(uint16_t) + m_DataSize + sizeof(int) * nReceiverCount;
//    m_pBuffer = new char[m_MaxSize];
//
//    int start_index = 0;
//    uint16_t packet_be16 = htons(static_cast<uint16_t>(m_PacketSize));
//    memcpy(m_pBuffer, &packet_be16, sizeof packet_be16);
//    start_index += sizeof packet_be16;
//
//    uint16_t data_be16 = htons(static_cast<uint16_t>(m_DataSize));
//    memcpy(m_pBuffer + start_index, &data_be16, sizeof data_be16);
//    start_index += sizeof data_be16;
//
//    start_index += sizeof int32_t;
//
...
//
//    int checkSum = static_cast<int>(
//        ::adler32(1, reinterpret_cast<const Bytef*>(m_pBuffer) + HEADER_SIZE, total_data_size));
//    checkSum = htonl(checkSum);
...
//}
//
//bool Packet::CheckValid()
//{
//    // uint16_t packet_size = 0;
//    // memcpy(&packet_size, m_pBuffer, sizeof(packet_size));
//    // m_PacketSize = ntohs(packet_size);
//    uint16_t data_size = 0;
//    memcpy(&data_size, m_pBuffer + sizeof(data_size), sizeof(data_size));
//    m_DataSize = ntohs(data_size);
//    int32_t expectedCheckSum =
//        static_cast<int>(::adler32(1, reinterpret_cast<const Bytef*>(m_pBuffer) + HEADER_SIZE,
//                                   m_DataSize + sizeof(uint16_t) - HEADER_SIZE - sizeof(int)));
//    int checkSum = 0;
//    memcpy(&checkSum, m_pBuffer + m_DataSize + sizeof(uint16_t) - sizeof(int), sizeof(checkSum));
//    checkSum = ntohl(checkSum);
//    return checkSum == expectedCheckSum;
//}
//
//bool Packet::AppendDestination(int dest_info)
//{
//    if ((m_PacketSize + sizeof(dest_info)) > m_MaxSize) {
//        return false;
//    }
//    memcpy(m_pBuffer + m_PacketSize, &dest_info, sizeof dest_info);
//    m_PacketSize += sizeof(dest_info);
//    uint16_t packet_be16 = htons(static_cast<uint16_t>(m_PacketSize));
//    memcpy(m_pBuffer, &packet_be16, sizeof packet_be16);
//    return true;
//}
//
//void Packet::GetAllDesination(int*& arrDest, int& nCount)
//{
//    assert(m_DataSize > 0);
//    nCount = (m_PacketSize - m_DataSize - sizeof(uint16_t)) / sizeof(int);
//    arrDest = reinterpret_cast<int*>(m_pBuffer + m_DataSize + sizeof(uint16_t));
//}
//
//void Packet::SetOwner(int owner_info)
//{
//    owner_info = htonl(owner_info);
//    memcpy(m_pBuffer + sizeof(uint16_t) + sizeof(uint16_t), &owner_info, sizeof owner_info);
//}
//int Packet::GetOwnerInfo()
//{
//    int owner_info = 0;
//    memcpy(&owner_info, m_pBuffer + +sizeof(uint16_t) + sizeof(uint16_t), sizeof owner_info);
//    return ntohl(owner_info);
//}
//
//void Packet::Shrink()
//{
//    uint16_t packet_be16 = htons(static_cast<uint16_t>(m_DataSize + sizeof(uint16_t)));
//    memcpy(m_pBuffer, &packet_be16, sizeof packet_be16);
//}
